...
CREATE TABLE time_entries_new (
    ...
    CHECK (
        billing_mode = 'hourly'
        OR (manual_amount IS NOT NULL AND manual_amount >= 0)
    )
);

INSERT INTO time_entries_new (
    id, user_id, project_id, invoice_id, date, start_time, end_time, duration_seconds, description, billable, invoiced, billing_mode, manual_amount
)
SELECT
    id, user_id, project_id, invoice_id, date, start_time, end_time, duration_seconds, description, billable, invoiced, 'hourly', NULL
FROM time_entries;
